...
const getAuthUser = async () => {
  ...
  if (!user) {
    throw new Error("You must be logged in to access thies route");
  }
...
export const updateProfileImageAction = async (
  ...
  try {
    ...
    revalidatePath('/profile');
    return { message: 'Profile image updated succesfully' };
  } catch (error) {
...
export const fetchFavoritedId = async ({ propertyId }: { propertyId: string; }) => {
  const user = await getAuthUser();
...
export const createReviewAction = async (prevState: any, formData: FormData) => {
  ...
  try {
    ...
    revalidatePath(`/properteis/${validatedFields.propertyId}`);
...
export const fetchStats = async () => {
  ...
  return {
    usersCount, propertiesCount, bookingsCount
  };
};
